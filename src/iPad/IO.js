...
IO.requestFromServer = function (url, whenDone) {
...
IO.getThumbnail = function (str, w, h, destw, desth) {
    str = str.replace(/>\s*</g, '><');
...
IO.getAsset = function (md5, fcn) { // returns either a link or a base64 dataurl
...
IO.getImagesInSVG = function (str, whenDone) {
    str = str.replace(/>\s*</g, '><');
    if (str.indexOf('xlink:href') < 0) {
...
IO.getImageDataURL = function (md5, data) {
    var header = '';
...
IO.getObject = function (md5, fcn) {
    if (md5.indexOf('/') > -1) {
        ...
        IO.getObjectinDB(IO.database, md5, fcn);
    }
};
...
IO.setMedia = function (data, type, fcn) {
    iOS.setmedia(btoa(data), type, fcn);
};

IO.query = function (type, obj, fcn) {
    var json = {};
...
IO.deleteobject = function (type, id, fcn) {
    var json = {};
...
IO.createProject = function (obj, fcn) {
    var json = {};
    ...
    json.stmt = 'insert into ' + IO.database + ' (' + keylist.toString() + ') values (' + values + ')';
    iOS.stmt(json, fcn);
...
IO.saveProject = function (obj, fcn) {
    ...
    json.stmt = 'update ' + IO.database + ' set ' + keylist.toString() + ' where id = ' + obj.id;
    iOS.stmt(json, fcn);
};
...
IO.setProjectIsGift = function (obj, fcn) {
    var json = {};
    var keylist = ['isgift = ?'];
...
IO.getExtension = function (str) {
    return str.substring(str.indexOf('.') + 1, str.length);
};

IO.getFilename = function (str) {
    return str.substring(0, str.indexOf('.'));
};

IO.parseProjectData = function (data) {
    var res = new Object();
    for (var key in data) {
...
IO.zipFile = null;
IO.zipAssetsExpected = 0;
IO.zipAssetsActual = 0;
IO.zipFileName = '';
IO.shareName = '';

IO.zipProject = function (projectReference, finished) {
    IO.getObject(projectReference, function (projectFromDB) {
        ...
        IO.zipFile = new JSZip();
        IO.zipFile.folder('project');

        var projectDataForZip = JSON.stringify(jsonData);
        IO.zipFile.file('project/data.json', projectDataForZip, {});
        ...
        var addMediaToZip = function (folder, md5) {
            var addB64ToZip = function (b64data) {
                IO.zipFile.file('project/' + folder + '/' + md5, b64data, {
                    base64: true,
                    createFolders: true
                });
                IO.zipAssetsActual++;
        ...
        // This could pause if getmedia takes a long time, for example, if we have many large sprites or large sounds

        IO.zipFileName = jsonData.name.replace(/\s*/g, '');
        IO.zipFileName = IO.zipFileName.replace(/\W/g, '');
        IO.shareName = jsonData.name;

        function checkStatus () {
            if ((IO.zipAssetsActual / IO.zipAssetsExpected) == 1) {
                finished(IO.zipFile.generate({
                    'compression': 'STORE'
                }));
            } else {
...
IO.uniqueProjectName = function (jsonData, callback, useOne) {
...
IO.loadProjectFromSjr = function (b64data) {
    // Together, these two provide a "progress" indication that lets us know when to refresh the lobby (when sE/sA = 1)
