var Layer = function () {};
Layer.targetOffscreen = document.createElement('canvas');
Layer.offscreen = document.createElement('canvas');

Layer.bringToFront = function (elem) {
    if (!elem) {
        return;
    }
...
Layer.bringElementsToFront = function () {
    var res = [];
...
Layer.onTopOf = function (p, index) {
    var res = [];
...
Layer.groupStartsAt = function (p, mt) {
    for (var i = 0; i < p.childElementCount; i++) {
...
Layer.onTopOfBy = function (p, mt, factor, n, list) {
    n = Math.max(0, n);
    Layer.drawInOffscreen(mt, Layer.targetOffscreen);
...
Layer.addFromBelow = function (p, mt, n, list) {
...
Layer.getRelated = function (elem) {
    var res = [];
...
Layer.inContactWith = function (p, mt, factor, n) {
    var res = [];
    for (var i = n; i < p.childElementCount; i++) {
        var elem = p.childNodes[i];
        if (elem.id == mt.id) {
...
Layer.includesBox = function (e1, e2) {
...
Layer.getContainedMost = function (p, elem, max, factor) {
    p = elem.parentNode;
    for (var i = 0; i < max; i++) {
...
Layer.overlapBox = function (e1, e2) {
    var box1 = SVGTools.getBox(e1);
    var box2 = SVGTools.getBox(e2);
...
Layer.overlapBoxBy = function (e1, e2, percent) {
    return Layer.overlapBox(e1, e2) >= percent;
};

Layer.findUnderMe = function (mt) {
    var p = gn('layer1');
...
//Layer.offscreen
Layer.verifyCollision = function (spr, other) {
    ...
    Layer.drawInOffscreen(other, Layer.offscreen);
    setCanvasSize(ScratchJr.workingCanvas, Paint.workspaceWidth, Paint.workspaceHeight);
    ...
    ctx.drawImage(Layer.targetOffscreen, 0, 0);
    ctx.restore();
...
Layer.drawInOffscreen = function (spr, cnv) {
...
Layer.isTransparent = function (data, node, w) {
    var dx = node.x;
    var dy = node.y;
    return data[(dx * 4) + dy * w * 4 + 3] == 0;
};

Layer.drawInContext = function (elem, ctx, zoom, lw, isTip) {
...
Layer.rotateFromCenter = function (ctx, group, angle) {
