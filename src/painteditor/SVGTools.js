...
SVGTools.attributePenTable;

SVGTools.init = function () {
    SVGTools.attributePenTable = SVGTools.getPenAttributes();
};

SVGTools.getPenAttributes = function () {
    return {
...
SVGTools.addPath = function (div, x, y) {
...
SVGTools.addPolyline = function (div, x, y) {
...
SVGTools.addEllipse = function (div, x, y) {
...
SVGTools.addTriangle = function (div, x, y) {
    var shape = document.createElementNS(Paint.xmlns, 'path');
    var attr = {
        'id': getIdFor('path'),
        'opacity': 1
...
SVGTools.addRect = function (div, x, y) {
...
SVGTools.rename = function (elem) {
    ...
    case 'g':
        ...
        break;
    case 'image':
        var corner = Transform.point(elem.getAttribute('x'), elem.getAttribute('y'), xform.matrix);
...
SVGTools.saveBackground = function (elem, w, h) {
...
SVGTools.cleanup = function (elem) {
...
SVGTools.saveShape = function (shape, w, h) {
...
SVGTools.calculateViewBox = function (elem) {
    ...
    box = box.expandBy(20);
    xform.setTranslate(-box.x, -box.y);
    Transform.translateTo(elem, xform);
    return box;
};

SVGTools.outsideBounds = function (box) {
    if (box.x < 0) {
        return true;
    }
    if (box.y < 0) {
...
SVGTools.notValidBox = function (box) {
...
SVGTools.getBoxCenter = function (elem) {
...
SVGTools.getTransformedBox = function (elem) {
...
SVGTools.getBox = function (elem, isSaving) {
...
SVGTools.getArea = function (elem) {
    var area = 0;
    var list;
...
SVGTools.getPolygon = function (before, here, size) {
...
SVGTools.polygonArea = function (list) {
    var xlist = [];
    var ylist = [];
...
SVGTools.getPathBox = function (elem) {
    var box;
...
SVGTools.getOnePathBox = function (d) {
...
SVGTools.onlyKeys = function (obj) {
    var res = [];
    for (var key in obj) {
        res.push(key);
    }
...
SVGTools.getDataurl = function (copy, w, h) {
    var serializer = new XMLSerializer();
...
SVGTools.getLayersAbove = function (p, index, w, h) {
    var serializer = new XMLSerializer();
...
SVGTools.cloneSVGelement = function (elem) {
    var group = Layer.findGroup(elem);
    var p = gn('layer1');
    if (!p) {
        return;
    }
    xform.setTranslate(5, 5);
    var old = [];
    var newlist = [];
...
SVGTools.removeDuplicates = function (list) {
    for (var i = 0; i < list.length; i++) {
        var mt = gn(list[i]);
        if (!mt) {
            continue;
...
SVGTools.getFlatten = function (p) {
    var res = [];
    for (var i = 0; i < p.childElementCount; i++) {
        var elem = p.childNodes[i];
...
SVGTools.getClonedElement = function (p, elem) {
...
SVGTools.getClone = function (p, elem) {
    ...
    Transform.translateTo(shape, xform);
    return shape;
};
...
SVGTools.getWatermark = function (shape, color) {
...
SVGTools.changeShape = function (svg, color) {
...
SVGTools.removeExtras = function (svg) {
    var n = 0;
...
SVGTools.setObjectWaterMark = function (elem, color) {
...
SVGTools.isCompoundPath = function (elem) {
    if (elem.tagName != 'path') {
        return false;
    }
